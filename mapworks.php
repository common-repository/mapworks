...
if(!$options['map_key']){
 $map_key = '';
 // AIzaSyD13npOOV1lqTY_TlukXuVCnEmKc7Et0AI
} else {
$map_key = $options['map_key'];
}
?>
<script src="https://maps.googleapis.com/maps/api/js?key=<?php echo $map_key;?>&v=3.exp&signed_in=true"></script>
<?php
}
...
function plugin_admin_add_page() {
	add_options_page('Mapworks Settings', 'Mapworks', 'manage_options', 'plugin', 'plugin_options_page');
}
...
add_settings_section('plugin_main', 'Mapworks settings', 'plugin_section_text', 'plugin');

add_settings_field('plugin_check_one', 'Display all locations in front', 'plugin_setting_check_one', 'plugin', 'plugin_main');
...
function plugin_options_one_validate($input) {
return $input;
}



function plugin_setting_check_one() {
$options = get_option('plugin_options');



echo "<input id='plugin_check_one' value='".$options['map_key']."' name='plugin_options[map_key]' size='75' type='text'/>";
} 
...
function shortcodephpmap($args, $content=""){
...
function shortcode_tagsmap($content){
...
add_filter('the_content',"shortcode_tagsmap");

add_shortcode('php',"shortcodephpmap");
add_shortcode('PHP',"shortcodephpmap");  
?>
